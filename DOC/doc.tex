...
\noindent I file \texttt{funzioni\_x\_msg} sono utilizzati sia dal client sia dal server e contiene le 
funzioni \texttt{send\_message()} e \\ 
\texttt{recv\_message()}, che implementano il meccanismo di invio e ricezione dei messaggi.
...
Paradigma \texttt{client-server multi-thread}. \
Il protocollo utilizzato è il \texttt{TCP} con \texttt{socket bloccanti}. l'utilizzo del TCP semplifica la gestione della comunicazione, in 
quanto garantisce affidabilità e ordine dei messaggi, inoltre qualora ci siano disconnessioni queste vengono rilevate. 
...
IL protocollo UDP non è stato preso in considerazione in quanto non permetteva di offrire ciò che offre il TCP a discapito della velocità. 
...
Questo approccio risolve il problema dei messaggi di lunghezza variabile evitando la necessità di delimitatori complessi. 
...
Gli eventuali segnali SIGPIPE che la send può emettere sono soppressi tramite la flag \texttt{MSG\_NOSIGNAL}, prevenendo crash del 
...
La gestione in memoria di queste strutture è dinamica, con allocazione e deallocazione in base alle esigenze; la scelta di una gestione dinamica 
...
La struttura info\_utente contiene le informazioni relative ad ogni utente connesso, come la porta, il socket descriptor (utilizzato per la comunicazione), 
...
da un mutex globale (\texttt{accesso\_lavagna}) che garantisce la mutua esclusione sulle operazioni di 
lettura/scrittura delle strutture condivise. Una variabile condition (\texttt{lavagna\_aggiornata}) viene utilizzata per implementare un meccanismo di notifica: 
...
La scelta del paradigma multi-thread è stata preferita rispetto all'I/O multiplexing per poter parallelizzare le richieste, e rispetto alla \texttt{fork()}, 
...
I thread vengono detached per permettere la deallocazione automatica delle risorse al termine, evitando di trasformarli in thread zombie.
...
e flag per tracciare l'invio di ACK\_CARD e CARD\_DONE. Questa struttura viene allocata dinamicamente quando viene assegnata una nuova card e deallocata al completamento 
o alla disconnessione. La scelta di mantenere localmente questi dati permette di validare i comandi dell'utente senza interrogare continuamente il server, riducendo 
il traffico di rete e migliorando la responsività dell'interfaccia. 
Sono inoltre presenti variabili globali volatile per tracciare lo stato del client: \texttt{hello\_eseguito} (indica se l'utente si è registrato), 
\texttt{ping\_ricevuto} (flag per il meccanismo di risposta al ping). 
L'uso di volatile garantisce che le modifiche a queste variabili siano immediatamente visibili tra i thread, prevenendo ottimizzazioni indesiderate del compilatore.
...
Quando un utente esegue il comando REVIEW\_CARD, richiede prima la lista degli utenti connessi al server per avere dati aggiornati. Per ogni utente nella lista, 
...
Per evitare invii duplicati, vengono utilizzati flag (\texttt{ack\_card\_inviata}, \texttt{card\_done\_inviata}) che vengono resettati ad ogni nuova card assegnata 
e controllati prima dell'invio del comando, impedendo comportamenti indesiderati causati da doppi invii.
...
\begin{itemize}
    ...
    \item \texttt{QUIT} - Disconnette l'utente dalla lavagna
\end{itemize}
\end{document}
